  ...
  virtual function void connect (this_type provider);
    ...
    if (provider == null) begin
      m_comp.uvm_report_error(s_connection_error_id,
                       $sformatf("Cannot connect to null port handle %s", get_full_name()), UVM_NONE); // CAVM
      return;
    end
...
"Cannot connect exports to ports Try calling port.connect(export) instead. (You attempted to bind this export to %s).", provider.get_full_name()), UVM_NONE);
  ...
  virtual function void resolve_bindings();
    ...
    if (size() < min_size() ) begin
      m_comp.uvm_report_error(s_connection_error_id, 
        $sformatf("connection count of %0d does not meet required minimum of %0d (%s)", // CAVM
        size(), min_size(), get_full_name()), UVM_NONE); // CAVM
    end
  
    if (max_size() != UVM_UNBOUNDED_CONNECTIONS && size() > max_size() ) begin
      m_comp.uvm_report_error(s_connection_error_id, 
        $sformatf("connection count of %0d exceeds maximum of %0d (%s)",  // CAVM
        size(), max_size(), get_full_name()), UVM_NONE); // CAVM
    end
