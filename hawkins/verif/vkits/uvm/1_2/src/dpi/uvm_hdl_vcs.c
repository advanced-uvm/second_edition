...
long int uvm_hdl_btoi(char *binVal) {
  // long int remainder, dec=0, j = 0;  CAVM: unused variable remainder
  long int dec=0, j = 0;
...
char *uvm_hdl_dtob(long int decimalNumber) {
   // int remainder, quotient;  CAVM: unused variable remainder
   int quotient;
  int  i=0,j, length;
  int binN[65];
  static char binaryNumber[65];
  // char *str = (char*) malloc(sizeof(char));  CAVM: Unused variable str
