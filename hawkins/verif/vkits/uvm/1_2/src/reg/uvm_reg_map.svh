...
function uvm_reg_map_info uvm_reg_map::get_reg_map_info(uvm_reg rg, bit error=1);
  ...
  if(!result.is_initialized)
    `uvm_warning("RegModel",{"map '",get_full_name(),"' does not seem to be initialized correctly, check that the top register model is locked()"}) // CAVM
  ...
  foreach (m_regs_info[rg_]) begin
   ...
   foreach (m_reg_arrays_info[rg_]) begin
      ...
      if(!m_reg_arrays_info[rg].unmapped) begin
         ...
         int bus_width = get_physical_addresses(m_reg_arrays_info[rg].offset,0,rg.get_size()*rg.get_default_reg().get_n_bytes(),addrs);
